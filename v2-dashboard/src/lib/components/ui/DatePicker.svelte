<script lang="ts">
	...
	let value: DateValue | undefined = $state();
	 ...
	 /**
	  * Check if the date is invalid (in the past).
	  * @param date
	  */
	 function isDateInvalid(date: DateValue) {
        // Compare the date with the start of today in the local timezone
        return date.compare(today(getLocalTimeZone())) < 0;
    }

    // This runs whenever 'value' changes.
    $effect(() => {
        if (value && isDateInvalid(value)) {
            console.warn("Attempted to select a past date. Resetting.");
            // If the selected date is invalid (past), reset it.
            // Option 1: Reset to today's date
            value = today(getLocalTimeZone());
            // Option 2: Reset to undefined (clears the input)
            // value = undefined;
        }
    });
</script>
...
<DatePicker.Root
    weekdayFormat="short"
    fixedWeeks={true}
    isDateDisabled={isDateInvalid}  bind:value                    >
    <div class="flex w-full flex-col gap-1.5">
        <DatePicker.Input {name} class={className}>
            {#snippet children({ segments })}
                {#each segments as { part, value: segValue }} <div class="inline-block select-none">
                        {#if part === 'literal'}
