        ...
        switch (eventType) {
            case 'user.created':
                ...
                if (!transactionID) {
                    ...
                    return new Response('Failed to process user creation: Could not start transaction', { status: 500 });
                }
                ...
                try {
                    ...
                    return new Response(JSON.stringify({ success: true, userId: payloadUserId }), { status: 201 }); // 201 Created
                    ...
                    payload.logger.error('Failed to process Clerk user.created webhook within transaction', error);
                    return new Response(JSON.stringify({ success: false, message: 'Failed to create user and organizer' }), { status: 500 });
                }
            ...
            case 'user.updated':
                ...
                try {
                    ...
                    if (!payloadUser) {
                        payload.logger.warn(`Received user.updated event for Clerk ID ${updatedClerkUser.id}, but no matching user found in Payload.`);
                        return new Response(JSON.stringify({ success: false, message: 'User not found in Payload' }), { status: 404 }); // 404 Not Found
                    }
                    ...
                    const newName = `${updatedClerkUser.first_name || ''} ${updatedClerkUser.last_name || ''}`.trim();
                    ...
                    if (Object.keys(updateData).length > 0) {
                        ...
                        console.log(`No relevant user data changes detected for Clerk ID ${updatedClerkUser.id}. No update performed.`);
                    }
                    ...
                    payload.logger.error(`Failed to process Clerk user.updated webhook for Clerk ID ${updatedClerkUser.id}`, error);
                    return new Response(JSON.stringify({ success: false, message: 'Failed to update user' }), { status: 500 });
                }
            ...
            case 'user.deleted':
                ...
                try {
                    ...
                    if (!payloadUserToDelete) {
                        payload.logger.warn(`Received user.deleted event for Clerk ID ${deletedClerkUser.id}, but no matching user found in Payload.`);
                        return new Response(JSON.stringify({ success: true, message: 'User not found, assumed already deleted' }), { status: 200 });
                    }
                    ...
                    console.log(`Successfully deleted Payload user ${payloadUserToDelete.id} corresponding to Clerk ID ${deletedClerkUser.id}.`);
                    // Return success response.
                    return new Response(JSON.stringify({ success: true }), { status: 200 }); // 200 OK

                } catch (error) {
                    // Handle any errors during the find or delete process.
                    payload.logger.error(`Failed to process Clerk user.deleted webhook for Clerk ID ${deletedClerkUser.id}`, error);
                    return new Response(JSON.stringify({ success: false, message: 'Failed to delete user' }), { status: 500 });
                }
