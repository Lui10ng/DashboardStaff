...
import { verifyToken, createClerkClient } from '@clerk/backend' // Removed unused createClerkClient for now
...
const clerkAuthStrategy: AuthStrategy = {
   ...
   * 4. **Attempt Primary Lookup:** Searches the Payload `users` collection for a
   ...
   *    If found, the user is authenticated.
   * 5. **Attempt Fallback Admin Lookup:** If no user is found via `clerkId`:
   *    a. Fetches the full Clerk user details using the Clerk User ID.
   *    b. Searches the Payload `users` collection for a document matching the
   *       Clerk user's primary email address AND having 'admin' in their
   *       `clerkRoles` field.
   *    c. (Requires 'users' collection to have 'email' and 'clerkRoles' fields).
   *    d. If such an admin user is found, they are authenticated.
   * 6. **Return Result:**
   *    - If a matching Payload user is found via *either* the `clerkId` lookup
   *      *or* the fallback email+admin lookup, returns `{ user: User }`
   *      (including the collection slug).
   *    - If the token is invalid, the `sub` claim is missing, or no matching
   *      Payload user is found by either method, returns `{ user: null }`.
   *
      ...
      try {
        ...
        if (clerkClaims?.sub) {
          ...
          if (docs && docs.length > 0) {
            ...
            // console.log(`Found matching Clerk user: ${JSON.stringify(clerkUser, null, 2)}`)
            ...
            if (adminUser.docs && adminUser.docs.length > 0) {
              const payloadUser = adminUser.docs[0]
              // console.log(`Found matching Admin user: ID ${payloadUser.id}`)
              // Add the collection slug to the user object for Payload compatibility
              return { user: { ...payloadUser, collection: 'users' } }
            }
